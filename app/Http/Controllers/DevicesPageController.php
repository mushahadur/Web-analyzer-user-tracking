...
class DevicesPageController extends Controller
{
    function indexPage($id){
        ...
        // get Device Info
        $AllDevice = VisitorTrackingLavel2t1::where('user_id',$userid)->where('domain_name',$domainName)->orderBy('id','desc')->select(VisitorTrackingLavel2t1::raw('user_id,visitor_ip,device,visitor_agent,time,date'))->get();
        $allDevice = json_decode($AllDevice);
    ...
    function test(){
        $userid = session()->get('userid');
        // get Device Info
        $AllPageLoadTime = VisitorTrackingLavel2t1::where('user_id',$userid)->orderBy('id')->select(VisitorTrackingLavel2t1::raw('user_id,visitor_ip,device,time,date'))->get();
