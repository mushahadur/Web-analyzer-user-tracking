...
class SocialNetworkPageController extends Controller
{
    function indexPage($id){
        ...
        $AllSocialNetwork = VisitorTrackingLavel2t1::where('user_id',$userid)->where('domain_name',$domainName)->orderBy('id')->select(VisitorTrackingLavel2t1::raw('user_id,visitor_ip,social_boot,time,date'))->get();
        $allSocialNetwork = json_decode($AllSocialNetwork);
        ...
        return view('Pages.socialNetworkPage',[
            'id'=>$id,
            'userDomainName'=>$domainName,
            'allSocialNetwork'=>$allSocialNetwork,
